...
export const sendResetPasswordEmail = async (to: string, token: string): Promise<void> => {
  ...
  const resetPasswordUrl = `${env.BASE_URL}/auth/verify-reset-password?token=${token}&newPassword=`
  const text = `Hi,
...
export const sendVerificationEmail = async (
  ...
  const verificationEmailUrl = `${env.BASE_URL}/auth/verify-email?token=${token}`
  const text = `Hi ${name},
